...
@test "all builds succeed with artifacts present" {
  ...
  export BUILD_RUST_RESULT="success"
  ...
  export RUST_ARTIFACTS='[{"name":"app"}]'
...
@test "result-json contains rust target" {
  export BUILD_RUST_RESULT="success"
  export RUST_ARTIFACTS='[{"name":"app"}]'
  ...
  run get_github_output "result-json"
  assert_output --partial '"rust":"success"'
}

@test "rust build failure propagates to stage result" {
  export BUILD_RUST_RESULT="failure"
  export RUST_ARTIFACTS='[{"name":"app"}]'
