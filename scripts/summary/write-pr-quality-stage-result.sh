...
main() {
  local commitlint_result licenselint_result dependencyreview_result sastopengrep_result megalint_result publiccodelint_result devbasecheck_result swift_result rust_result
  ...
  rust_result="$(ci_normalize_result "${RUST_RESULT:-skipped}")"

  local stage_result
  stage_result="$(ci_aggregate_results "$commitlint_result" "$licenselint_result" "$dependencyreview_result" "$sastopengrep_result" "$megalint_result" "$publiccodelint_result" "$devbasecheck_result" "$swift_result" "$rust_result")"
  ...
  targets_json="$(ci_build_targets_json \
    ...
    "rust:$(effective_result "${RUST_ENABLED:-false}" "$rust_result")")"
