...
main() {
  local maven_result npm_result gradle_result gradleandroid_result xcodeios_result rust_result
  ...
  rust_result="$(ci_normalize_result "${BUILD_RUST_RESULT:-skipped}")"

  local stage_ran=false
  if [[ "${MAVEN_ARTIFACTS:-[]}" != '[]' || "${NPM_ARTIFACTS:-[]}" != '[]' || "${GRADLE_ARTIFACTS:-[]}" != '[]' || "${GRADLEANDROID_ARTIFACTS:-[]}" != '[]' || "${XCODEIOS_ARTIFACTS:-[]}" != '[]' || "${RUST_ARTIFACTS:-[]}" != '[]' ]]; then
    stage_ran=true
  fi

  local stage_result
  stage_result="$(ci_stage_result "$stage_ran" "$maven_result" "$npm_result" "$gradle_result" "$gradleandroid_result" "$xcodeios_result" "$rust_result")"

  local targets_json result_json
  targets_json="$(ci_build_targets_json "maven:$maven_result" "npm:$npm_result" "gradle:$gradle_result" "gradleandroid:$gradleandroid_result" "xcodeios:$xcodeios_result" "rust:$rust_result")"
